...
# class FAQSearch:
#     def __init__(self, faqs_file="Backend/data/faqs.json"):
#         self.faqs_file = faqs_file
#         self.model = SentenceTransformer('all-MiniLM-L6-v2')
#         self.faq_questions = []
#         self.faq_answers = []
#         self.index = None
#         self._load_faqs_and_build_index()

#     def _load_faqs_and_build_index(self):
#         if not os.path.exists(self.faqs_file):
#             print(f"⚠ FAQ file {self.faqs_file} not found.")
#             return
...
#                     self.faq_answers.append(answer)

#         if self.faq_questions:
#             embeddings = self.model.encode(self.faq_questions, convert_to_numpy=True, show_progress_bar=True)
#             dimension = embeddings.shape[1]
#             self.index = faiss.IndexFlatL2(dimension)
#             self.index.add(embeddings)
#             print(f"✅ Loaded and indexed {len(self.faq_questions)} FAQs.")
#         else:
#             print("⚠ No FAQ questions found to index.")

#     def search(self, query, top_k=1, threshold=0.5):
#         if not self.index or not self.faq_questions:
#             return []
...
#                     else:
#                         print(f"⚠ Skipped non-dictionary item in section '{section}': {item}")
#             else:
#                 print(f"⚠ Skipped non-list section '{section}'")
...
# # Global instances for app.py to import
